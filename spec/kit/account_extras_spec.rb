...
RSpec.describe "Kit::Resources::Account extras" do
  ...
  describe "#email_stats" do
    it "returns the raw stats hash" do
      stub_kit(:get, "/v4/account/email_stats", body: { "stats" => { "sent" => 5, "open_rate" => 0.4 } })
      expect(client.account.email_stats).to eq("sent" => 5, "open_rate" => 0.4)
    end
  end

  describe "#growth_stats" do
    it "passes starting/ending and returns the raw stats hash" do
      stub = stub_request(:get, "https://api.kit.com/v4/account/growth_stats")
             .with(query: { "starting" => "2026-01-01" })
             .to_return(status: 200, headers: { "Content-Type" => "application/json" },
                        body: JSON.generate("stats" => { "subscribers" => 100 }))
      expect(client.account.growth_stats(starting: "2026-01-01")).to eq("subscribers" => 100)
      expect(stub).to have_been_requested
    end
  end
end
